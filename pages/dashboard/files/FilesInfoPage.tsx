...
import {
  FileSizeType,
  FileType,
} from "@/components/app/dashboard/latest_uploads/types/app";
...
import { FileOptions, FilesFilterKey } from "@/components/types";
...
const FilesInfoPage = () => {
  ...
  function uploadFile(): void | false {
    if (!fileInfo.fileName || !fileInfo.fileUploader || !fileInfo.fileSize) {
      toast.error(
        `${fileInfo.fileType === "jsx" ? "Component" : "File"} can't be empty !`
      );
      dispatch(
        pushNotificationAction(
          `${
            fileInfo.fileType === "jsx" ? "Component" : "File"
          } can't be empty.`
        )
      );
      return false;
    ...
    toast.success(
      `${fileInfo.fileType === "jsx" ? "Component" : "File"} ${
        fileInfo.fileName
      }.${fileInfo.fileType} uploaded successfully !`
    );
    dispatch(
      pushNotificationAction(
        `${fileInfo.fileType === "jsx" ? "Component" : "File"} ${
      ...
      <BoxContainer className="flex flex-col col-span-3 gap-4">
        ...
        <Input
          ...
          onChange={(e) =>
            setFileInfo((prevState) => ({
              ...prevState,
              fileSize: parseFloat(e.target.value),
            }))
          }
        />
        <Select
          ...
          onChange={(e) =>
            setFileInfo((prevState) => ({
              ...prevState,
              fileSizeType: e.target.value as FileSizeType,
            }))
          }
        >
          <option
        ...
        <Select
          ...
          onChange={(e) =>
            setFileOptions((prevState) => ({
              filterKey: e.target.value as FilesFilterKey,
              filterValue: "",
            }))
          }
        >
