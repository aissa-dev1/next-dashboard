...
import { TaskOptions, TasksFilterKey } from "@/components/types";
...
const TasksInfoPage = () => {
  ...
  function addTask(e: React.FormEvent<HTMLFormElement>): void | false {
    ...
    if (taskOptions.content.length < 2) {
      ...
      dispatch(
        pushNotificationAction("Task can't be less than two characters.")
      );
      return false;
    }
    ...
    for (const item of state.tasks) {
      if (
        item.content.match(taskOptions.content) &&
        item.content.length === taskOptions.content.length
      ) {
    ...
    dispatch(
      pushNotificationAction(`Task ${taskOptions.content} added successfully.`)
    );
        ...
        <Flex className="gap-2" direction="col">
          <Select
            ...
            onChange={(e) =>
              setTaskOptions((prevState) => ({
                ...prevState,
                filterKey: e.target.value as TasksFilterKey,
              }))
            }
          >
            <option
